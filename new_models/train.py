    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            precip_weights = torch.tensor([get_precip_weight(p.item()) for p in precip_target.view(-1)],
                                        device=device)
            precip_loss = criterion[1](precip_pred.squeeze(), precip_target) * precip_weights.mean()
            ...
            correlation_loss = F.mse_loss(rain_probs, expected_rain_probs)
...
def main():
    ...
    # 构建数据文件的绝对路径
    data_path = os.path.join(current_dir, '..', 'weather_data', 'data_w', '1y.csv')  
    print(f"\n正在加载数据文件: {data_path}")
